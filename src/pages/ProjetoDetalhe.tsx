...
export default function ProjetoDetalhe() {
  ...
  const carregarDados = async () => {
    ...
    try {
      ...
      const [projs, regs, subs, fas] = await Promise.all([
      ...
      // Inicializar estado de expansão das fases (fases com >0h nascem expandidas, 0h recolhidas)
  ...
  // Total contratado: soma das fases se houver fases com horas, senão horas contratadas do projeto
  ...
  // Última atividade calculada explicitamente
    ...
    <div className="min-h-screen bg-[#0B0E14] text-white flex flex-col lg:flex-row">
      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto max-w-6xl lg:ml-[240px] space-y-6 w-full">
        ...
        <div>
          <button
            onClick={() => navigate(-1)}
            className="inline-flex items-center gap-2 text-sm font-semibold text-gray-400 hover:text-white transition-colors focus:outline-none"
          >
        ...
        {loading ? (
          ...
          <div className="bg-[#161B22] border border-gray-800 rounded-2xl p-12 text-center max-w-lg mx-auto space-y-4">
            ...
            <p className="text-sm text-gray-400">
              O projeto solicitado não foi localizado ou não está mais disponível.
            </p>
            <button
              onClick={() => navigate(-1)}
            ...
            <div className="bg-[#161B22] border border-gray-800 rounded-2xl p-6 space-y-3">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <h1 className="text-2xl font-bold text-white tracking-tight uppercase truncate">
                    {projeto.nome}
                  </h1>
                </div>
              ...
              <div className="flex justify-between items-center text-sm font-medium">
                {temContratado ? (
                  <p className="text-[#8B949E]">
                    <span className="text-white font-bold">{totalLancado.toFixed(2).replace('.', ',')}h</span> lançadas de{' '}
                    {totalContratado!.toFixed(2).replace('.', ',')}h contratadas
                  </p>
                ) : (
                  <p className="text-[#8B949E]">
                ...
                {temContratado && (
                  <span
                    ...
                    style={{ color: excedeuContratado ? '#F44336' : '#4CAF50' }}
                  >
                    {percentualGeral}%
                  </span>
                )}
                ...
                <div className="w-full bg-[#0B0E14] h-[6px] rounded-full overflow-hidden border border-gray-800/50">
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor: excedeuContratado ? '#F44336' : '#4CAF50'
                    }}
                  />
                </div>
              )}
              ...
              {fases.length > 0 ? (
                <div className="space-y-4">
                  {fases.map((fase) => {
                    ...
                    // Mapeamento das subcategorias da fase
                    const subsMapeadas = subsDaFase.map(sub => {
                      const duracao = regsDaFase
                    ...
                    const subcategoriasComPercentual = [...comDuracao, ...semDuracao].map(s => ({
                      ...s,
                      percentual: usadoFase > 0 ? Math.round((s.duracao / usadoFase) * 100) : 0
                    }))

                    const horasContratadasFormatadas = fase.horas_contratadas !== null && fase.horas_contratadas > 0
                      ? `${fase.horas_contratadas.toFixed(2).replace('.', ',')}h`
                      : '—'
                    ...
                    return (
                      <div key={fase.id} className="bg-[#161B22] border border-gray-800 rounded-2xl overflow-hidden shadow-sm">
                        <button
                          ...
                          className="w-full flex items-center justify-between p-5 hover:bg-[#1E2530]/40 transition-colors focus:outline-none"
                        >
                          <div className="flex items-center gap-3 min-w-0">
                            <svg
                          ...
                          <div className="font-mono text-sm font-semibold text-[#8B949E]">
                            <span className="text-white font-bold">{usadoFase.toFixed(2).replace('.', ',')}h</span>
                            {' / '}
                            {horasContratadasFormatadas}
                          </div>
                        </button>
                  ...
                  {/* Se houver subcategorias sem fase ou registros sem subcategoria */}
                  {(() => {
                    const subIdsComFase = new Set(subcategorias.filter(s => s.fase_id !== null).map(s => s.id))
                    const regsOrfaos = registros.filter(r => !r.subcategoria_id || !subIdsComFase.has(r.subcategoria_id))
                    const subsOrfas = subcategorias.filter(s => s.fase_id === null)

                    if (regsOrfaos.length === 0 && subsOrfas.length === 0) return null

                    const usadoOrfao = regsOrfaos.reduce((acc, r) => acc + r.duracao, 0)
                    const duracaoSemSub = regsOrfaos
                      .filter(r => !r.subcategoria_id || !subcategorias.some(s => s.id === r.subcategoria_id))
                      .reduce((acc, r) => acc + r.duracao, 0)

                    const subsMapeadas = subsOrfas.map(sub => {
                      const duracao = regsOrfaos
                        .filter(r => r.subcategoria_id === sub.id)
                        .reduce((acc, r) => acc + r.duracao, 0)
                    ...
                    const lista: Array<{ id: string | null; nome: string; duracao: number; horas_alocadas: number | null }> = [...comDuracao, ...semDuracao]
                    if (duracaoSemSub > 0) {
                      lista.push({ id: null, nome: 'Sem subcategoria', duracao: duracaoSemSub, horas_alocadas: null })
                    }

                    const items = lista.map(s => ({
                      ...s,
                      percentual: usadoOrfao > 0 ? Math.round((s.duracao / usadoOrfao) * 100) : 0
                    }))

                    return (
                      <div className="bg-[#161B22] border border-gray-800 rounded-2xl p-5 space-y-3">
                        <h3 className="font-bold text-gray-400 text-sm">Sem Fase</h3>
                        <BreakdownSubcategorias subcategorias={items} />
                      </div>
                    )
                  })()}
                </div>
                ...
                <div className="bg-[#161B22] border border-gray-800 rounded-2xl p-6">
                  {(() => {
                    ...
                    const items = lista.map(s => ({
                      ...s,
                      percentual: totalLancado > 0 ? Math.round((s.duracao / totalLancado) * 100) : 0
                    }))

                    return <BreakdownSubcategorias subcategorias={items} />
                  })()}
                </div>
              )}
            </div>
